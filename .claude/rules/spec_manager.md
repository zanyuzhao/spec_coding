...
- **specs/**：Source of Truth。按领域分目录（如 `specs/auth/`、`specs/spec_list/`），每领域一份 `spec.md` 描述**当前系统**已合入的行为与约定。归档时必须把本次变更按 delta 合并进对应 `specs/<领域>/spec.md`。领域名与 spec 文件名一律使用**下划线**，见 `domain_naming_convention.md`。
...
3. **命名**：`{领域}_{简短描述}.md` 或目录 `{领域}_{描述}/`，如 `auth_login.md`、`spec_list_api.md`。一律使用**下划线**，禁止中划线；详见 `domain_naming_convention.md`。
