...
- [ ] （可选）已编写单元测试或集成测试（详见「[测试最佳实践](./testing_best_practices.md)」）。
- [ ] **文档/注释**与行为一致：关键接口有 docstring 或说明（如请求/响应示例），README 或 `docs/` 若有更新则与实现一致。

### 1.5 契约一致性校验

- [ ] **API 契约一致性**：spec 中的每个端点（路径、方法、参数）与后端路由实现完全一致。
- [ ] **请求格式一致性**：后端接收的参数类型、必填性与 spec 请求格式一致。
- [ ] **响应格式一致性**：后端返回的 JSON 结构、字段名、类型与 spec 响应格式一致。
- [ ] **分页格式一致性**：若涉及分页，返回结构符合 spec 约定的分页格式（items/total/page/size/pages）。
- [ ] **前端类型一致性**：前端 TypeScript 类型定义与后端 Pydantic 模型字段一一对应。
- [ ] **字段对照表核对**：spec 中的字段对照表与实际代码实现一致（字段名、类型、必填性）。

### 1.6 功能完整性校验

- [ ] **所有端点已实现**：spec API 契约中列出的所有端点在后端有对应路由。
- [ ] **所有页面已实现**：spec 中描述的前端页面/组件已实现。
- [ ] **所有场景已覆盖**：spec 用户故事/场景中的关键流程可正常执行。
- [ ] **错误处理完整**：边界条件、非法输入、权限校验等有对应错误响应。

### 1.7 E2E 验证（需求完成后执行）

- [ ] **环境已启动**：`./docs/plan_auto/init.sh` 前后端服务正常运行。
- [ ] **关键流程已验证**：在浏览器中完成 spec 用户故事描述的关键流程（如：登录 → 列表 → 详情 → 操作）。
- [ ] **前后端集成正常**：前端页面能正确调用后端 API 并显示数据。
- [ ] **边界条件已测试**：空数据、错误输入、分页越界等场景表现正常。

### 1.8 实现完成判定

- [ ] 以上各项通过后，可认定**实现完成**：spec 中列出的接口/场景/约定在代码中均有对应且行为一致，E2E 验证通过。
...
- [ ] **E2E 验证已通过**：开发清单 1.7 中的 E2E 验证已完成且通过。
...
- **规则引用**：`spec_manager.md` 中已约定开发时参考本开发清单、归档时按本归档检查列表执行；本文件为常青文档，存放在 `docs/spec_process/`，不随单次需求归档而删除。
