...
- **触发（仅文档阶段）**：新功能、修改既有功能/行为（如「加一个登录」「订单列表要分页」）→ 若需求/边界尚不清晰，先用 **Plan 模式** 做一轮多轮澄清（见 `docs/spec_process/SPEC_PLAN_GUIDE.md`），再在 `docs/spec/active/` 下产出**需求说明 + 设计文档**，然后**停止**，等用户确认或修改文档；**不**在未确认前自动写代码。
...
**阶段一：提需求 → Plan 澄清（可选但推荐）→ 出文档，然后停**

1. 说新功能或改需求，例如：「加一个订单列表分页」。
2. 若需求范围/边界不清晰，优先使用原生 **/plan** 或「开始规划」按钮，按 `docs/spec_process/SPEC_PLAN_GUIDE.md` 中的提问框架补齐关键信息。
3. Plan 阶段结束后，将达成共识的决策整理为要点，在 `docs/spec/active/` 下生成**需求说明 + 设计文档**（见 `docs/spec_process/SPEC_DOC_TEMPLATE.md`），然后**停止**，请你查看或修改文档，不会自动写代码。
