...
### 2. ECC + Plan-Auto（工具与长线自治）

- **ECC（Everything Claude Code）**：引入其**验证环、TDD、code-review、安全与 API 设计**等能力，以 **Skills** 形式接入；plan/verify/tdd/code-review 内嵌到「继续开发」与 Plan-Auto 轮次，无需人类单独敲命令。详见 [everything-claude-code](https://github.com/affaan-m/everything-claude-code)。
- **Plan-Auto（借鉴 Effective harnesses）**：提供 **Plan-Auto 模式**——先产出计划（feature list），再自动增量实现，每步 E2E 验收，适合「人类少介入、AI 多轮产出可交付结果」的场景。详见 [Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)。

本框架将 **Spec 流程以 Cursor/Claude 的 Rules + Skills 落地**：何时走 Spec、何时走 Plan-Auto、如何归档由规则与技能驱动；**双模式**可在一仓内并存，按需求选择「精细控制」或「快速产出」。

---

## 项目思路

- **双模式**：新需求/改需求时可选 **A) Spec 流程**（先出需求与设计文档，确认后再实现，适合精细控制与维护老代码）或 **B) Plan-Auto 流程**（AI 先出计划再自动增量实现并 E2E 验收，适合快速验证与少介入）。
...
- **规则 (rules)**：`spec_trigger` 决定何时走需求流程、是否给出 Spec/Plan-Auto 选项、继续开发时走哪条 Skill；`spec_manager` 定义归档与 Delta 合并；`global_guard`、`fastapi_shield`、`frontend_architect` 为全局与栈规范；`ecc_testing`、`ecc_security` 为测试与安全约束。
- **技能 (skills)**：
  - **流程类**：`spec_implementation_phase`（Spec 下继续开发：plan → TDD → verify → code-review）、`plan_auto_initializer`（Plan-Auto 首轮）、`plan_auto_coding_session`（Plan-Auto 每轮一 feature）。
...
- **docs/spec_process/**：流程说明、开发/归档检查清单、测试最佳实践。
- **docs/plan_auto/**：Plan-Auto 模式的启动脚本、验证脚本、MCP 配置文档。

### 可安装化

- 在任意项目根执行 `spec-coding init` 即可接入本框架（含 `docs/spec/`、`.cursor/`、`.claude/`），无需复制整仓；支持 `--backend-dir`、`--frontend-dir`、`--app-package` 与 `--docs-only`。
...
- **若未指定模式**：AI 会给出选项，请你选 **A) Spec 流程** 或 **B) Plan-Auto 流程**。
- **若已指定**：如「用 Spec 做」「走 Plan-Auto」则直接按该模式执行。
...
### 3. Plan-Auto 模式（先计划再自动实现）

1. **首轮（Plan）**：AI 执行 `plan_auto_initializer`，产出高层范围、`feature_list.json`、`init.sh`、`claude-progress.txt` 及初始提交。
2. **后续轮（Auto）**：你说「继续」或「下一轮」，AI 执行 `plan_auto_coding_session`：选一个未完成 feature → 跑 init + 基线 E2E → 实现 → verify（含 E2E）→ 通过则标 passes，更新 progress 并提交；循环直到全部完成。
...
接入后得到 `docs/spec/`、`docs/spec_process/`、`docs/plan_auto/`、`.cursor/rules/`、`.cursor/skills/`、`.claude/rules/`、`.claude/skills/`、`CLAUDE.md`，业务目录名可自定义。
...
- `docs/spec_process/` — 流程说明、开发/归档检查清单、测试最佳实践
- `docs/plan_auto/` — Plan-Auto 模式的启动脚本、验证脚本、MCP 配置
- `.cursor/rules/` — 框架规则（spec_trigger、spec_manager、global_guard、fastapi_shield、frontend_architect、ecc_testing、ecc_security）
- `.cursor/skills/` — 流程与约定技能（spec_implementation_phase、plan_auto_*、api_pydantic_style、domain_naming_convention 及 ECC 系列）
...
- 使用其他技术栈时，可保留 Spec/Plan-Auto 流程与规则，由 AI 协助替换或增补对应栈的 skills（如 ECC 的 Python/TypeScript 规则与技能）。
...
配置文件：`.cursor/mcp.json`。详细说明见 `docs/plan_auto/mcp_setup.md`。
