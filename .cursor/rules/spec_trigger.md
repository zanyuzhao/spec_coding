...
在开始任何代码修改前，必须先检查当前分支：

```bash
git branch --show-current
```

- 若当前在 `main`、`master`、`sit`、`dev`、`develop`、`staging`、`production` 等主分支：
  - **必须先创建新分支**：`git checkout -b feature/<需求名>`
  - 回复示例：「检测到当前在 main 分支，已自动创建 feature/xxx 分支进行开发。」
- 若已在 feature 分支且是同一需求的连续修改：可直接继续

**前置检查二：归档状态**

在开始新需求前，先执行 `./scripts/archive.sh --check` 检查是否有可归档的已完成需求。若有，提示用户：
- 「检测到已完成的未归档需求：[列表]。是否先归档？[Y] 归档后继续 [N] 直接开始」
