...
#
# 用法:
#   ./scripts/services/postgres.sh create [name]
#   ./scripts/services/postgres.sh status [name]
#   ./scripts/services/postgres.sh stop [name]
...
# 配置
# ========================================
CONTAINER_PREFIX="dev-postgres"
DEFAULT_PORT_START=5432
DEFAULT_PORT_END=5499
...
create_postgres() {
    ...
    # 检查健康状态
    for i in {1..30}; do
        if docker exec "$container_name" pg_isready -U "$user" &>/dev/null; then
...
EOF
    ...
    log_info "✅ PostgreSQL service created!"
    echo ""
...
status_postgres() {
    ...
    echo -e "${CYAN}=== PostgreSQL Service Status ===${NC}"
...
main() {
    ...
    case "$command" in
        ...
        stop)
            ...
            ;;
        status)
            status_postgres "$name"
            ;;
        remove|rm)
            remove_postgres "$name"
            ;;
        *)
